 ...
 */
static inline int
__las_insert_update(WT_SESSION_IMPL *session, WT_CURSOR *cursor, const WT_SAVE_UPD *list,
  const uint32_t btree_id, const WT_ITEM *key, const WT_UPDATE *upd, const uint8_t type,
  const WT_ITEM *las_value, WT_BIRTHMARK_DETAILS *birthmarkp, uint64_t *prepared_insert_cntp)
{
    cursor->set_key(cursor, btree_id, key, upd->start_ts, upd->txnid);

    /*
     * If saving a non-zero length value on the page, save a birthmark instead of duplicating it in
     * the lookaside table. (We check the length because row-store doesn't write zero-length data
     * items.)
     */
    if (upd->txnid == list->onpage_upd.txnid && upd->start_ts == list->onpage_upd.start_ts &&
      upd->size > 0 && (upd->type == WT_UPDATE_STANDARD || upd->type == WT_UPDATE_MODIFY)) {
...
/*
 * __las_insert_full_update --
 *     insert a single full update to lookaside
 */
static inline int
__las_insert_full_update(WT_SESSION_IMPL *session, WT_CURSOR *cursor, const WT_SAVE_UPD *list,
  const uint32_t btree_id, const WT_ITEM *key, const WT_UPDATE *upd,
  WT_BIRTHMARK_DETAILS *birthmarkp, uint64_t *prepared_insert_cntp)
{
    WT_ITEM las_value;
    ...
    WT_RET(__las_insert_update(session, cursor, list, btree_id, key, upd, WT_UPDATE_STANDARD,
      &las_value, birthmarkp, prepared_insert_cntp));
...
{
    static WT_ITEM TOMBSTONE_VALUE = {NULL, 0, NULL, 0, 0};
...
#define MAX_REVERSE_MODIFY_NUM 16
    ...
    WT_UPDATE *first_upd, *upd, *prev_upd, *stack_upd, *prev_stack_upd;
    wt_off_t las_size;
    ...
    for (i = 0, list = multi->supd; i < multi->supd_entries; ++i, ++list) {
         ...
         * aborted items (there's no point to restoring them).
         *
         * The algorithm assumes that we can always find a full update after a series of consecutive
         * modifies and the update newer to a tombstone must be a full update.
         *
         * It walks the update chain and pushes all the modifies to a stack until we see a full
         * update. When we see a full update, we first flush out all the updates on the stack. In
         * this part, we first calculate the full value of the updates using the forward deltas,
         * then calculate the reverse delta by comparing the newer update's full value and older
         * update's full value, and finally write the reverse delta or the full value to lookaside.
         * Also we skip all the updates except the first one on chain with the same start timestamp
         * and transaction id. After all this, we push the full update to the stack if its start
         * timestamp or transaction id is different to the newer update. We skip everything deep in
         * the chain with the same start timestamp and transaction id.
         *
         * If we see a tombstone, we know it is either the first update on chain or the previous
         * update we see is a full update. Therefore, we should only have maximum one update on
         * stack, which is a full update. We flush that update if it exists and then write the
         * tombstone to lookaside. In addition, we skip everything after it with the same start
        ...
        for (upd = first_upd; upd != NULL; upd = upd->next) {
            ...
            case WT_UPDATE_MODIFY:
                /* Push the modifies to the stack */
            ...
            case WT_UPDATE_TOMBSTONE:
                ...
                /* The update happens after a tombstone if exists must be a full update */
                WT_ASSERT(session, modifies.size == 0 || modifies.size == 1);
                /* Flush the only full update on the stack */
                if (modifies.size > 0) {
                    __wt_modify_vector_pop(&modifies, &stack_upd);
                    WT_ASSERT(session, stack_upd->type == WT_UPDATE_STANDARD);
                    WT_ERR(__las_insert_full_update(session, cursor, list, btree_id, key, stack_upd,
                      birthmarkp, &prepared_insert_cnt));
                    ++insert_cnt;
                }

                /* Write the tombstone to lookaside */
                WT_ERR(__las_insert_update(session, cursor, list, btree_id, key, upd,
                  WT_UPDATE_TOMBSTONE, &TOMBSTONE_VALUE, birthmarkp, &prepared_insert_cnt));
                ++insert_cnt;

                /* Skip following updates with the same start timestamp and transaction id */
                while (upd->next != NULL && upd->next->start_ts == upd->start_ts &&
                  upd->next->txnid == upd->txnid) {
                    squashed = true;
                    upd = upd->next;
                }
            ...
            case WT_UPDATE_STANDARD:
                ...
                /* Calculate the full value of the first update on the stack */
                if (modifies.size > 0) {
                    __wt_modify_vector_pop(&modifies, &stack_upd);
                    prev_upd = stack_upd;
                    WT_ERR(__las_convert_full_update(
                      session, stack_upd, upd->data, upd->size, full_value));
                }

                /* Flush the updates on stack */
                for (; modifies.size > 0; tmp = full_value, full_value = prev_full_value,
                                          prev_full_value = tmp, stack_upd = prev_stack_upd) {
                    __wt_modify_vector_pop(&modifies, &prev_stack_upd);
                    WT_ERR(__las_convert_full_update(session, prev_stack_upd, full_value->data,
                      full_value->size, prev_full_value));

                    /*
                     * Skip updates have the same start timestamp and transaction id
                     *
                     * Calculate reverse delta and flush
                     */
                    if (stack_upd->start_ts != prev_stack_upd->start_ts ||
                      stack_upd->txnid != prev_stack_upd->txnid) {
                        nentries = MAX_REVERSE_MODIFY_NUM;
                        if (__wt_calc_modify((WT_SESSION *)session, prev_full_value, full_value,
                              prev_full_value->size / 10, entries, &nentries) != 0)
                            WT_ERR(
                              __las_insert_update(session, cursor, list, btree_id, key, stack_upd,
                                WT_UPDATE_STANDARD, full_value, birthmarkp, &prepared_insert_cnt));
                        else {
                            WT_ERR(__wt_modify_pack(cursor, &modify_value, entries, nentries));
                            if (__las_insert_update(session, cursor, list, btree_id, key,
                                  prev_stack_upd, WT_UPDATE_MODIFY, modify_value, birthmarkp,
                                  &prepared_insert_cnt) != 0) {
                ...
                /*
                 * We always write the full value for the last update on the stack because it only
                 * can be the first update on chain, a modify before a tombstone, or a full update
                 * originally
                 */
                if (stack_upd != NULL) {
                    WT_ERR(__las_insert_update(session, cursor, list, btree_id, key, stack_upd,
                      WT_UPDATE_STANDARD, full_value, birthmarkp, &prepared_insert_cnt));
                    ++insert_cnt;
                }

                /*
                 * Push to the stack if its start timestamp and transaction id are different to the
                 * prev_upd
                 */
                if (prev_upd == NULL || prev_upd->start_ts != upd->start_ts ||
                  prev_upd->txnid != upd->txnid)
                    WT_ERR(__wt_modify_vector_push(&modifies, upd));
        ...
        /*
         * To this point, the modifies stack should either be empty if the last update is skipped or
         * the last update is a tombstone, or only have one element left which is a full update.
         */
        WT_ASSERT(session, modifies.size == 0 || modifies.size == 1);
        if (modifies.size > 0) {
            __wt_modify_vector_pop(&modifies, &stack_upd);
            WT_ASSERT(session, stack_upd->type == WT_UPDATE_STANDARD);
            WT_ERR(__las_insert_full_update(
              session, cursor, list, btree_id, key, stack_upd, birthmarkp, &prepared_insert_cnt));
            ++insert_cnt;
        }
    }
